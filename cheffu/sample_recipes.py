SampleRecipes = {
    'Magic Mushroom Powder': {
        ...
        'procedure': [
            ...
            # Move the Mixture in the previous System (the pulverized mushrooms from the spice grinder) into the jar
            ...
            # Perhaps instead of asking for a specific tool, we can ask for a Tool that matches a certain interface, such as 'MIXER' or 'STIRRER'?
            { 'TOOL': 'spoon', },
            ...
            # This will use the bound spoon, and remove it from the bundle and stack (since it is a Tool, not an Appliance)
            # When acting upon a bundle, Verbs will attempt these rules in order, stopping at and using the first available result:
            #     1) The topmost Tool in the bundle's Tool stack; if this occurs, the Tool is popped off the stack and discarded
    ...
    'Chamomile Tea Pound Cake': {
        ...
        'procedure': [
            ...
            { 'ADDI': None, },
            { 'ENVR': 'stove', },
            # The Precondition is used for making sure Environments or Appliances are preset correctly
            # This is mainly useful for initial steps that may require a non-trivial amount of time, such as oven preheating
            { 'PREC': 'high', },
            { 'PLAC': None, },
            { 'VERB': 'boil', },
            ...
            # Strain the tea
            # The Pseudo command "selects" a user-described food portion of the last bundle, and makes a reference to it on the stack
            # **OLD** This does NOT actually physically separate the portion from its source (see below how to do that)!
            # **OLD** Performing a Verb on the reference modifies that portion in-place where it was found
            # **OLD** Performing a Discard on the reference removes it from its source AND from the stack
            # **OLD** If discarding is not desired, the reference can be re-integrated with a Push (which only works on references)
            # **OLD** In addition, a reference can be physically separated from its souce using a Move, Transfer, or Add (into a new Mixture)
            { 'PSEU': 'used tea', },
            { 'VERB': 'strain', },
            # While straining the tea, squeeze it to release extra liquid
            # The Simultaneous token must follow a Verb, Load, or Add token, and indicates another Verb action that should be done simultanouesly with the former
            { 'SIMU': 'squeeze', },
            { 'DISC': None, }, # **OLD** If we wanted to keep using the tea bags for some reason, we could do a Push or Move/Transfer/Add instead of a Discard
            ...
            # The 'keep' argument represents the part to leave behind, and the 'pass' is the amount to pull out
            # Both must be greater than 0
            # **OLD** Similar to Pseudo, the Divided pass-portion is kept as a reference until it needs to be separated
            { 'DIVI': { 'keep': 1, 'pass': 1, }, },
            ...
            # Right now, we just assume that the 'zest' Verb turns the entire lemon into a pile of zest
            # Is there a better way to represent this for when we'd want to use the zest AND the fruit?
            { 'VERB': 'zest', },
            { 'QVOL': { 'base': 2, 'units': 'tsp', }, },
            { 'ADDI': None, },
            ...
            # Add the flour mixture and reserved tea, in three installments each, alternating between flour and tea
            [
                { 'TGET': 'FLOUR_MIXTURE', },
                { 'ADDI': None, },
                { 'VERB': 'mix', },
